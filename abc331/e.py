...
for i in range(L):
    c,d = MI()
    excluded_combos.append((c,d))

def find_highest_priced_set_efficient(N, M, main_dishes, side_dishes, excluded_combinations):
    # 除外された組み合わせのためのセットを作成
    excluded_set = {(c - 1, d - 1) for c, d in excluded_combinations}
    ...
    # 全ての組み合わせが除外されている場合（理論上はあり得ないが、念のため）
    return -1

max_price = find_highest_priced_set_efficient(N, M, a, b, excluded_combos)
print(max_price)