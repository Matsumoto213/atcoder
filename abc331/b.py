...
for six_pack in range((N // 6) + 2):
    for eight_pack in range((N // 8) + 2):
        for twelve_pack in range((N // 12) + 2):
            total_eggs = 6 * six_pack + 8 * eight_pack + 12 * twelve_pack
            total_cost = S * six_pack + M * eight_pack + L * twelve_pack
            if total_eggs >= N:
