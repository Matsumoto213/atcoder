...
N = II()
def find_nth_repunit_sum_expanded(n):
    repunits = [int("1" * i) for i in range(1, 16)]
    sums = set()
    for r1 in repunits:
        for r2 in repunits:
            for r3 in repunits:
    ...
    return sorted_sums[n-1] if n < len(sorted_sums) else "N is out of range"
ans = find_nth_repunit_sum_expanded(N)
print(ans)