...
def check_if_root_is_leaf(n, edges):
...
def min_operations_to_remove_root_optimized(n, edges):
    ...
    def dfs(node, parent):
        ...
        for child in graph[node]:
            if child != parent:
                ...
                descendants_count[node] += 1 + descendants_count[child]

    subtree_size = [0] * (n + 1)
    descendants_count = [0] * (n + 1)
    dfs(1, -1)

    nodes_sorted = sorted(range(2, n + 1), key=lambda x: (descendants_count[x], subtree_size[x]), reverse=True)

    operations = 0
    removed = set()
    for node in nodes_sorted:
        if subtree_size[node] == 1 and node not in removed:
            current = node
            while current != 1 and current not in removed:
                removed.add(current)
                current = next((v for v in graph[current] if v in graph and v != 1), 1)
            operations += 1
...
is_root_leaf = check_if_root_is_leaf(N, edges)
min_operations = 1 if is_root_leaf else min_operations_to_remove_root_optimized(N, edges)
