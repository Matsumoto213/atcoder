...
for i in range(K):
    if not has_odd_sock and i == max_diff_index:
        continue
    if i == 0 or (not has_odd_sock and i == max_diff_index + 1):
        total_weirdness += A[i] - 1
    elif i == K - 1 or (not has_odd_sock and i == max_diff_index - 1):
        total_weirdness += N - A[i]
    else:
        total_weirdness += (A[i] - A[i - 1] - 1) // 2
