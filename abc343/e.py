...
# 与えられた体積から可能な辺の長さの組を生成する関数
def find_cube_dimensions(volume):
    # 体積の立方根の近似値を取得（上限値）
    approx_root = int(volume ** (1/3)) + 1
    # 1から近似値までの数で体積を因数分解し、辺の長さの組を見つける
    for factors in combinations_with_replacement(range(1, approx_root + 1), 3):
        if factors[0] * factors[1] * factors[2] == volume:
            return factors
    return None  # 適切な組が見つからない場合

dimensions1 = find_cube_dimensions(V1)
dimensions2 = find_cube_dimensions(V2)
dimensions3 = find_cube_dimensions(V3)

# 判定結果を出力する
# 各体積に対する辺の長さの組が見つかったかどうかをチェック
if dimensions1 and dimensions2 and dimensions3:
    # C1 <= C2 <= C3 の関係を満たす必要があるため、各立方体の最大の辺の長さを比較
    max_edge1, max_edge2, max_edge3 = max(dimensions1), max(dimensions2), max(dimensions3)
    if max_edge1 <= max_edge2 <= max_edge3:
        print("Yes")
        # 各辺の長さの差を用いて、a, b, cの組を出力
        # ここでは仮に体積から求めた辺の長さをそのままa, b, cとして出力
        print(*dimensions1, *dimensions2, *dimensions3)
    else:
        print("No")
else:
    print("No")