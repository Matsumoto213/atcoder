...
def is_revers_string(num):
    return str(num) == str(num)[::-1]

def is_valid_palindromic(number):
    number_str = str(number)
    return all(int(digit) <= 9 for digit in number_str[:-1])

x = int(N ** (1/3))

for i in range(x, 0, -1):
        cube = i ** 3
        # Check if the cube is a palindrome and satisfies the new condition
        if is_revers_string(cube) and is_valid_palindromic(cube):
            print(cube)