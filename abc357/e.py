...
def count_reachable_vertices(N, a):
    # グラフの逆向きの隣接リストを作成
    reverse_graph = [[] for _ in range(N + 1)]
    for i in range(1, N + 1):
        reverse_graph[a[i - 1]].append(i)

    # 各頂点の到達可能な頂点の数を格納するリスト
    reachable_count = [0] * (N + 1)
    
    # 深さ優先探索（DFS）を使って到達可能な頂点をカウントする
    def dfs(v):
        ...
        for u in reverse_graph[v]:
            count += dfs(u)
        reachable_count[v] = count
        return count
    
    # すべての頂点に対してDFSを実行して到達可能な頂点数を計算
    for i in range(1, N + 1):
...
# 到達可能な頂点の数を計算
ans = count_reachable_vertices(N, A)
