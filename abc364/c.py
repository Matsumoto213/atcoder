...
def calculate_min_dishes(dishes, max_value, is_sweetness=True):
    total = 0
    count = 0
...
dishes_by_sweetness = sorted(zip(A, B), key=lambda x: x[0], reverse=True)
dishes_by_saltiness = sorted(zip(A, B), key=lambda x: x[1], reverse=True)

min_by_sweetness = calculate_min_dishes(dishes_by_sweetness, X, is_sweetness=True)
min_by_saltiness = calculate_min_dishes(dishes_by_saltiness, Y, is_sweetness=False)

print(min(min_by_sweetness, min_by_saltiness))
