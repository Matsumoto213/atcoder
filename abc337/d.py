...
def min_operations_to_form_o_sequence(grid, H, W, K):
    def count_dots_for_o_sequence(line, length):
        count_o = 0
        count_dot = 0
        min_dots = float('inf')
        ...
        for i in range(length):
            ...
            if i >= K - 1 and count_o + count_dot >= K:
                min_dots = min(min_dots, K - count_o)
    ...
    for row in grid:
        min_operations = min(min_operations, count_dots_for_o_sequence(row, W))

    for col in range(W):
        col_str = ''.join(grid[row][col] for row in range(H))
        min_operations = min(min_operations, count_dots_for_o_sequence(col_str, H))

    return min_operations if min_operations != float('inf') else -1

print(min_operations_to_form_o_sequence(S, H, W, K))