...
def dfs(graph, node, target, A, visited, current_path, max_score):
    if node == target:
        max_score[0] = max(max_score[0], len(set(current_path)))
        return

    visited[node] = True
    for neighbor in graph[node]:
        if not visited[neighbor]:
            if A[neighbor - 1] >= current_path[-1]:
                dfs(graph, neighbor, target, A, visited, current_path + [A[neighbor - 1]], max_score)
    visited[node] = False
...
dfs(graph, 1, N, A, visited, [A[0]], max_score)
print(max_score[0])